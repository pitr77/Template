...
// Check slider status.
add_filter( 'musical_vibe_filter_slider_status', 'musical_vibe_check_slider_status' );

// Add slider to the theme.
add_action( 'musical_vibe_action_before_content', 'musical_vibe_add_featured_slider', 5 );

// Slider details.
add_filter( 'musical_vibe_filter_slider_details', 'musical_vibe_get_slider_details' );


if ( ! function_exists( 'musical_vibe_get_slider_details' ) ) :
	/**
	 * Slider details.
	 *
	 * @since 1.0.0
	 ...
	 */
	function musical_vibe_get_slider_details( $input ) {

		$featured_slider_type           = musical_vibe_get_option( 'featured_slider_type' );
		$featured_slider_number         = musical_vibe_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = musical_vibe_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = musical_vibe_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$item['id'] = absint( $id );
						$item['caption_alignment'] = musical_vibe_get_option( 'featured_slider_page_caption_alignment_' . $i );
						$blocks[ $item['id'] ] = $item;
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							$image_array = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'musical-vibe-slider' );
							$slides[ $cnt ]['images']  = $image_array;
							$slides[ $cnt ]['title']   = esc_html( $post->post_title );
							$slides[ $cnt ]['url']     = esc_url( get_permalink( $post->ID ) );
							$slides[ $cnt ]['excerpt'] = musical_vibe_the_excerpt( apply_filters( 'musical_vibe_filter_slider_caption_length', 30 ), $post );
...
if ( ! function_exists( 'musical_vibe_add_featured_slider' ) ) :
	/**
	 * Add featured slider.
	 *
	 * @since 1.0.0
	 */
	function musical_vibe_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'musical_vibe_filter_slider_status', false );
		if ( true !== $flag_apply_slider ) {
			return false;
		}

		$slider_details = array();
		$slider_details = apply_filters( 'musical_vibe_filter_slider_details', $slider_details );
		...
		// Render slider now.
		musical_vibe_render_featured_slider( $slider_details );

	}
endif;

if ( ! function_exists( 'musical_vibe_render_featured_slider' ) ) :
	/**
	 ...
	 */
	function musical_vibe_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}

		$featured_slider_transition_effect   = musical_vibe_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = musical_vibe_get_option( 'featured_slider_enable_caption' );
		$featured_slider_caption_alignment   = musical_vibe_get_option( 'featured_slider_caption_alignment' );
		$featured_slider_enable_arrow        = musical_vibe_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = musical_vibe_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = musical_vibe_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_enable_overlay      = musical_vibe_get_option( 'featured_slider_enable_overlay' );
		$featured_slider_transition_duration = musical_vibe_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = musical_vibe_get_option( 'featured_slider_transition_delay' );
...
if( ! function_exists( 'musical_vibe_check_slider_status' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function musical_vibe_check_slider_status( $input ) {

		// Slider status.
		$featured_slider_status = musical_vibe_get_option( 'featured_slider_status' );
